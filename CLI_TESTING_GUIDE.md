...
To test loan default:

1. **Request another loan** (loan_id: 1)
2. **Get vouches** (Alice vouches)
3. **Wait for loan term + grace period** (or manually advance time in test)
4. **Call check_default**:
   - Contract: `LOAN_MANAGER_ADDRESS`
   - Message: `check_default`
   - Args: `1`
5. **Verify**:
   - Loan status: `Defaulted`
   - Charlie's stars: reduced
   - Alice's stars: lost staked stars (10)
   - Alice's deposit: slashed by staked capital

## Using Polkadot.js API (Programmatic)

For programmatic testing, use the Polkadot.js API:
...
async function test() {
  ...
  const contract = new ContractPromise(
    api,
    contractAbi, // Load from .json file
    contractAddress
  );
