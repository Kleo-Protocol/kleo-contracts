...
mod loan_manager {
    ...
    impl LoanManager {
        ...
        #[ink(message)]
        pub fn request_loan(&mut self, amount: Balance, loan_term: Timestamp) -> Result<AccountId> {
            if amount == 0 {
                return Err(Error::ZeroAmount);
            }
            let caller: AccountId = self.get_converted_caller();
    ...
    //     #[ink(message)]
    //     pub fn vouch_for_loan(&mut self, loan_id: u64, stars: u32, capital_percent: u8) -> Result<()> {
    ...
    //         let voucher = self.get_converted_caller();
    ...
    //     #[ink(message, payable)]
    //     pub fn repay_loan(&mut self, loan_id: u64) -> Result<()> {
    //         let mut loan = self.loans.get(loan_id).ok_or(Error::LoanNotFound)?;
    ...
    //     #[ink(message)]
    //     pub fn check_default(&mut self, loan_id: u64) -> Result<()> {
    //         let mut loan = self.loans.get(loan_id).ok_or(Error::LoanNotFound)?;
